...
import org.bynar.versailles.TypeExpression
...
trait BitTypeExpression extends TypeExpression {
...
case class BitRecordComponent(val name: Symbol, val `type`: TypeExpression) extends Statement {
    
    override type SelfTerm = BitRecordComponent
    
    def copy(name: Symbol = name, `type`: TypeExpression = `type`) =
